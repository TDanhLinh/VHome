...
export default function AddNewFinance(props) {
    ...
    const validationSchema = Yup.object().shape({
        ...
        servicesId: Yup.string().required("Vui lòng chọn dịch vụ"),
        ...
        paidMoney: Yup.number().required("Vui lòng nhập số tiền đã thanh toán")
            .max(Yup.ref('money'), "Số tiền thanh toán không được vượt quá số tiền"), // Limit based on money field
    });
            ...
            addAPI(functionName, finance)
                .then((response) => {
                    ...
                    if (response.status === 201) {
                        ...
                        alert("Thêm hóa đơn mới thất bại");
                        setOpen(false);
                    }
                })
                .catch((error) => {
    ...
    return (<Container fixed>
        <Grid container columnSpacing={3}>
            <Grid item xs={12}>
                ...
                />
                <Autocomplete id="outlined-basic2" sx={{m: 1, width: "100%"}} options={customer}
                ...
                <TextField sx={{m: 1, width: "100%"}} id="outlined-basic4" label="Tòa" variant="outlined"
                           ...
                           InputLabelProps={{shrink: true}}
                />
                <Autocomplete id="outlined-basic5" sx={{m: 1, width: "100%"}} options={services}
                              ...
                              onChange={(event, newValue) => {
                                  setFinance({...finance, servicesId: newValue.id, money: newValue.price});
                              }}
                              renderInput={(params) => (
            ...
            <Grid item xs={6}>
                <TextField sx={{m: 1, width: "100%"}} id="outlined-basic7" label="Tiền đã trả" variant="outlined"
                           ...
                           error={!!errors.paidMoney} helperText={errors.paidMoney}
                />
            </Grid>
